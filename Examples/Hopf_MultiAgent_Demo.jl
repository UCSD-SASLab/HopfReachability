...
using .HopfReachability: Hopf_minT, Hopf_BRS, Hopf_admm_cd, Hopf_admm, Hopf_cd, intH_mrk18, preH_mrk18, plot_BRS, HJoc_mrk18
...
x = [x_init, y_init_offset, x_vel, y_vel_init, x_init + 1e-3, -y_init_offset, x_vel, y_vel_init]

uˢ, dˢ, Tˢ, ϕ = Hopf_minT(system, targets[1], x; opt_method=Hopf_cd, opt_p=opt_p_cd, preH=preH_mrk18, intH=intH_mrk18, HJoc=HJoc_mrk18, time_p, printing=true, warm=true)

# uˢ, dˢ, Tˢ, ϕ = Hopf_minT(system, targets[1], x; opt_method=Hopf_admm_cd, preH=preH_mrk18, intH=intH_mrk18, HJoc=HJoc_mrk18, time_p, printing=true, warm=false)

## Optimally Controlled Evolution
...
Δt, tf = 0.5, 30.0
X = zeros(dim_xh, Int(tf / Δt) + 1)
X[:, 1] = [x_init, y_init_offset, x_vel, y_vel_init, x_init + 1, -y_init_offset, x_vel, y_vel_init]
catcher = zeros(Int(tf / Δt))
...
for (ti, th) in enumerate(0.:Δt:tf-Δt)
    ...
    uˢ1, dˢ1, Tˢ1, ϕ1 = Hopf_minT(system, targets[1], X[:, ti]; opt_method=Hopf_cd, opt_p=opt_p_cd, preH=preH_mrk18, intH=intH_mrk18, HJoc=HJoc_mrk18, time_p, printing=false, warm=false)
    uˢ2, dˢ2, Tˢ2, ϕ2 = Hopf_minT(system, targets[2], X[:, ti]; opt_method=Hopf_cd, opt_p=opt_p_cd, preH=preH_mrk18, intH=intH_mrk18, HJoc=HJoc_mrk18, time_p, printing=false, warm=false)

    uˢ, dˢ, catcher[ti] = Tˢ1 < Tˢ2 ? (uˢ1, dˢ1, 1) : (uˢ2, dˢ2, 2)
...
plot(title="Relative Trajectories"); 
plot!(X[1,:], X[2,:], label="agent 1", color=:blue); 
plot!(X[5,:], X[6,:], label="agent 2", color=:orange)

plot(title="Relative Vertical Velocities"); 
plot!(collect(0.:Δt:tf), X[4,:], label="agent 1", color=:blue); 
plot!(collect(0.:Δt:tf), X[8,:], label="agent 2", color=:orange)

...
def path(time, phase): return jnp.cos((2.*jnp.pi/2.) * time + phase)
...
# def avoid_values_axes(t): return -jnp.minimum(jnp.minimum(
#                             jnp.array(jnp.max(jnp.abs(jnp.multiply(diag, jnp.subtract(grid.states[..., :2], ci(0) + jnp.array([0., path(t, phases[0])])))), axis=-1)) - rAvoid, 
#                             jnp.array(jnp.max(jnp.abs(jnp.multiply(diag, jnp.subtract(grid.states[..., :2], ci(1) + jnp.array([0., path(t, phases[1])])))), axis=-1)) - rAvoid),
#                             jnp.array(jnp.max(jnp.abs(jnp.multiply(diag, jnp.subtract(grid.states[..., :2], ci(2) + jnp.array([0., path(t, phases[2])])))), axis=-1)) - rAvoid)
...
def BRT_pp(t,v): return BRT(t, v, reach_values)
...
def BRAT_ball_pp(t,v): return BRAT(t,v, reach_values, avoid_values_ball, 0.)
...
def BRAAT_ball_lin_pp_l0(t,v): return BRAAT(t, v, BAT_values_lin_ball_full, times, reach_values(t), avoid_values_ball(t), 0.)
def BRAAT_ball_nlin_pp_l0(t,v): return BRAAT(t, v, BAT_values_lin_ball_full, times, reach_values(t), avoid_values_ball(t), 0.)

BRAAT_values_lin, BRAAT_fig = solveplot(diffgame_lin, init_values_reachavoid_ball, BRAAT_ball_lin_pp_l0, title="Conveyor BRAAT Ball (Linear)", plot_reach_solid=True, plot_avoid_solid=True, plot_bcs=True, plot_no_value=True)

BRAAT_values_nlin, BRAAT_fig = solveplot(diffgame_nlin, init_values_reachavoid_ball, BRAAT_ball_nlin_pp_l0, title="Conveyor BRAAT Ball (Linear)", plot_reach_solid=True, plot_avoid_solid=True, plot_bcs=True, plot_no_value=True)

# %% BRAAT Lambda Variation

lam = -0.25

def BRAAT_ball_lin_pp_lmp25(t,v): return BRAAT(t, v, BAT_values_lin_ball_full, times, reach_values(t), avoid_values_ball(t), lam)
def BRAAT_ball_nlin_pp_lmp25(t,v): return BRAAT(t, v, BAT_values_nlin_ball_full, times, reach_values(t), avoid_values_ball(t), lam)

BRAAT_values, BRAAT_fig = solveplot(diffgame_lin, init_values_reachavoid_ball_lam(lam), BRAAT_ball_lin_pp_lmp25, title=f"Conveyor BRAAT Ball - Lambda {lam:2.1f} (Linear)", plot_reach_solid=False, plot_avoid_solid=False, plot_bcs=True, plot_no_value=False)
BRAAT_values_nlin, BRAAT_fig = solveplot(diffgame_nlin, init_values_reachavoid_ball_lam(lam), BRAAT_ball_nlin_pp_lmp25, title=f"Conveyor BRAAT Ball - Lambda {lam:2.1f} (Nonlinear)", plot_reach_solid=False, plot_avoid_solid=False, plot_bcs=True, plot_no_value=False)

# %%

lam = 0.1

def BRAAT_ball_lin_pp_lp1(t,v): return BRAAT(t, v, BAT_values_lin_ball_full, times, reach_values(t), avoid_values_ball(t), lam)
def BRAAT_ball_nlin_pp_lp1(t,v): return BRAAT(t, v, BAT_values_nlin_ball_full, times, reach_values(t), avoid_values_ball(t), lam)

BRAAT_values_lin, BRAAT_fig = solveplot(diffgame_lin, init_values_reachavoid_ball_lam(lam), BRAAT_ball_lin_pp_lp1, title=f"Conveyor BRAAT Ball - Lambda {lam:2.1f} (Linear)", plot_reach_solid=False, plot_avoid_solid=False, plot_bcs=True, plot_no_value=False)
BRAAT_values_nlin, BRAAT_fig = solveplot(diffgame_nlin, init_values_reachavoid_ball_lam(lam), BRAAT_ball_nlin_pp_lp1, title=f"Conveyor BRAAT Ball - Lambda {lam:2.1f} (Nonlinear)", plot_reach_solid=False, plot_avoid_solid=False, plot_bcs=True, plot_no_value=False)
...
def solve_BRAATpanel(diffgame, init_values_lam, vAsol, lams, reach_value_fn, avoid_value_fn, ti=0., tf=TC, ntimes=100):
    ...
    for li, lam in enumerate(lams):
        ...
        def BRAAT_post_processor(t,v, timez=timez, avoid_sol=avoid_sol, reach_fn=reach_fn, avoid_fn=avoid_fn, lamb=lam_fixed):
...
## Linear
BRAAT_values_lam_lin, times = solve_BRAATpanel(diffgame_lin, init_values_reachavoid_ball_lam, BAT_values_lin_ball_full, lams, reach_values, avoid_values_ball) #, tf=0.5)

## Nonlnear
BRAAT_values_lam_nlin, times = solve_BRAATpanel(diffgame_nlin, init_values_reachavoid_ball_lam, BAT_values_nlin_ball_full, lams, reach_values, avoid_values_ball) #, tf=0.5)
...
def BRAT_axes_pp(t,v): return BRAT(t,v, reach_values, avoid_values_axes, 0.)

BAT_values_lin_axes_full, BAT_axe_fig = solveplot(diffgame_lin, init_values_avoid_axes, BRAT_axes_pp, title="Conveyor BAT Axes (Linear)", plot_avoid_solid=False, plot_no_value=False, plot_aBC=True, avoid_game=True, one_shot=True, ntimes=100, avoid_values=avoid_values_axes, offset=0)
BAT_values_nlin_axes_full, BAT_axe_fig = solveplot(diffgame_nlin, init_values_avoid_axes, BRAT_axes_pp, title="Conveyor BAT Axes (Nonlinear)", plot_avoid_solid=False, plot_no_value=False, plot_aBC=True, avoid_game=True, one_shot=True, ntimes=100, avoid_values=avoid_values_axes, offset=0)

# %% AXES - BRAAT/BRAT

lam = -1.
def BRAAT_axes_lin_pp_l(t,v): return BRAAT(t, v, BAT_values_lin_axes_full, times, reach_values(t), avoid_values_ball(t), lam)
def BRAAT_axes_nlin_pp_l(t,v): return BRAAT(t, v, BAT_values_lin_axes_full, times, reach_values(t), avoid_values_ball(t), lam)

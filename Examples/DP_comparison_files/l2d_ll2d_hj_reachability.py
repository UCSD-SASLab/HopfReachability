...
class Linear2D(hj.ControlAndDisturbanceAffineDynamics):
    def __init__(self, 
        ...
        if control_space is None:
            control_space = hj.sets.Box(lo=jnp.array([-max_u, -max_u]), hi=jnp.array([max_u, max_u]))
    
        if disturbance_space is None:
            disturbance_space = hj.sets.Box(lo=jnp.array([-max_d, -max_d]), hi=jnp.array([max_d, max_d]))
    ...
    def disturbance_jacobian(self, state, time):
        return jnp.array([
            [self.c1, 0.], # FIXME: made the edit, need the test
            [0., self.c2],
        ])
...
# Note, the following is designed to be a decomposition (and thus has 1 input)
class Linear2D_1input(hj.ControlAndDisturbanceAffineDynamics):
    def __init__(self, 
                a11=-0.5, a12=0., a21=-1., a22=-0.5,
                b=.4, c=.1,
                max_u=0.5,
                max_d=0.3,
                control_mode="min",
...
# Note, the following is designed to be a decomposition (and thus has 1 input)
class LessLinear2D_1input(hj.ControlAndDisturbanceAffineDynamics):
    def __init__(self, 
                a11=-0.5, a12=0., a21=-1., a22=-0.5,
                b=.4, c=.1,
                max_u=0.5,
    ...
    def open_loop_dynamics(self, x, time):
        xdot1 = self.a11 * x[0] + self.a12 * x[1] + self.mu * jnp.sin(self.alpha * x[0]) * x[0] * x[0]
        xdot2 = self.a21 * x[0] + self.a22 * x[1] - self.gamma * x[1] * x[0] * x[0]
        return jnp.array([xdot1, xdot2])

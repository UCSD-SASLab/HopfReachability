...
lbs = np.concatenate(([-(IH + eps)], -x_width * np.ones(N)))
grid_L = 501
...
rAvoid = 0.3
c = jnp.array([-1.0, 0.])
avoid_values_ball = lambda t: -(jnp.linalg.norm(jnp.subtract(grid.states[..., :2], c), axis=-1) - rAvoid)
...
def solveplot(diffgame, init_values, post_processor, title="", tf=TC, ntimes=5, 
                ...
                one_shot=False, avoid_game=False, offset = 2):
...
BAT_values_full, BAT_fig = solveplot(diffgame_lin, init_values_avoid_ball, lambda t,v: BAT(t, v, avoid_values_ball), title="Conveyor BAT Ball (Linear)", plot_avoid_solid=False, plot_no_value=False, plot_aBC=True, avoid_game=True, one_shot=True, ntimes=ntimes)

# %% BRAAT

BRAAT_values, BRAAT_fig = solveplot(diffgame_lin, init_values_reachavoid_ball, lambda t,v: BRAAT(t, v, BAT_values_full, times, reach_values(t), avoid_values_ball(t), 0.), title="Conveyor BRAAT Ball (Linear)", plot_reach_solid=True, plot_avoid_solid=True, plot_bcs=True, plot_no_value=True)

# %% BRAAT Lambda Variation

lam = -3.
BRAAT_values, BRAAT_fig = solveplot(diffgame_lin, init_values_reachavoid_ball_lam(lam), lambda t,v: BRAAT(t, v, BAT_values_full, times, reach_values(t), avoid_values_ball(t), lam), title=f"Conveyor BRAAT Ball - Lambda {lam:2.1f} (Linear)", plot_reach_solid=False, plot_avoid_solid=False, plot_bcs=True, plot_no_value=False)
...
BRAAT_values, BRAAT_fig = solveplot(diffgame_lin, init_values_reachavoid_ball_lam(lam), title="BRAAT\n" + r"$\max\left\{\min_{\tau\in[t,T]} \:\: R(x(\tau)) + \lambda, \max_{\tau'\in[t,T]}\:\: A(x(\tau'))\right\}- max\{λ, 0\}$" + f"\n λ={lam:2.1f}", plot_bcs=True, one_shot=True)
...
lams=[-3., -0.5, 0., 0.2, 3.0]

## Linear
BRAAT_values_lam_lin, times = solve_BRAATpanel(diffgame_lin, init_values_reachavoid_ball_lam, BAT_values_full, lams) #, tf=0.5)

## Nonlnear
BRAAT_values_lam_nlin, times = solve_BRAATpanel(diffgame_nlin, init_values_reachavoid_ball_lam, BAT_values_full, lams) #, tf=0.5)

# %%

def plot_BRAATpanel(values_lams, lams, times, i, reach_values=reach_values, avoid_values=avoid_values_ball):
...
def anim_BRAATpanel(values_lams, lams, times, i, reach_values=reach_values, avoid_values=avoid_values_ball, bounce=False):
    ...
    if not bounce:
        ...
        N = len(times)
        bounce_frames = list(range(N)) + list(range(N-2, -1, -1))
...
ntimes = 100
times = -np.linspace(0., TC, ntimes)
BAT_values_axes_full, BAT_axe_fig = solveplot(diffgame_lin, init_values_avoid_axes, lambda t,v: BAT(t, v, avoid_values_axes), title="Conveyor BAT Axes (Linear)", plot_avoid_solid=False, plot_no_value=False, plot_aBC=True, avoid_game=True, one_shot=True, ntimes=ntimes, avoid_values=avoid_values_axes, offset=0)

# %% AXES - BRAAT/BRAT

lam = 0.
BRAAT_values_axes, BRAAT_fig = solveplot(diffgame_lin, init_values_reachavoid_axes_lam(lam), lambda t,v: BRAAT(t, v, BAT_values_axes_full, times, reach_values(t), avoid_values_axes(t), lam), title=f"Conveyor BRAAT Ball - Lambda {lam:2.1f} (Linear)", plot_bcs=True, avoid_values=avoid_values_axes)
...
## Linear
BRAAT_values_axes_lam_lin, times = solve_BRAATpanel(diffgame_lin, init_values_reachavoid_axes_lam, BAT_values_axes_full, lams, avoid_values=avoid_values_axes) #, tf=0.5)

## Nonlnear
BRAAT_values_axes_lam_nlin, times = solve_BRAATpanel(diffgame_nlin, init_values_reachavoid_axes_lam, BAT_values_axes_full, lams, avoid_values=avoid_values_axes) #, tf=0.5)

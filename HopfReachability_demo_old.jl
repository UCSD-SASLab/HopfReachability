...
include("/Users/willsharpless/Library/Mobile Documents/com~apple~CloudDocs/Herbert/Koop_HJR/HL_fastHJR/HopfReachability_old.jl");
using .HopfReachability_old: Hopf_BRS, Hopf_admm, Hopf_cd, intH_ytc17, preH_ytc17, plot_BRS, Hopf
...
cp = [0.; 0.]
r = 1.0
J(x::Vector, A, c) = ((x - c)' * A * (x - c))/2 - 0.5 * r^2 #don't need yet
Js(v::Vector, A, c) = (v' * inv(A) * v)/2 + c'v + 0.5 * r^2
J(x::Matrix, A, c) = diag((x .- c)' * A * (x .- c))/2 .- 0.5 * r^2
Js(v::Matrix, A, c) = diag(v' * inv(A) * v)/2 + (c'v)' .+ 0.5 * r^2 #don't need yet
target = (J, Js, (Ap, cp))
...
# Ap = nothing
# cp = [0.; 0.]
# J(x::Vector, c) = ((x - c)' * (x - c))/2 #don't need yet
# Js(v::Vector, c) = (v' * v)/2 + c'v
# J(x::Matrix, c) = diag((x .- c)' * (x .- c))/2
# Js(v::Matrix, c) = diag(v' * v)/2 + (c'v)' #don't need yet
# target = (J, Js, (Ap, cp))
...
plot = plot_BRS(T, B⁺T, ϕB⁺T; M, cres=0.1, ϵc=0.1, ϵs=0.025, contour=true)
# plot = plot_BRS(T, B⁺T, [ϕB⁺Ti .+ 0 for ϕB⁺Ti in ϕB⁺T]; M, cres=0.1, contour=false)
...
# plot = plot_BRS(T, B⁺T, ϕB⁺T; M, cres=0.01, contour=true, title="BRT of T, in X");